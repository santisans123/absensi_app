...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Future<void> _fetchReportData() async {
    try {
      ...
      final AbsentReport absentReport =
          AbsentReport.fromJson(result['data'] as Map<String, dynamic>);
      setState(() {
  ...
  void _onDaySelected(DateTime day, DateTime focusedDay) {
    final List<dynamic> eventsData = _getEventsForDay(day);
    final List<dynamic> holidaysData = _getHolidayForDay(day);
    setState(() {
      if (eventsData is List<DailyData>) {
        _selectedEvents = eventsData;
      } else {
        _selectedEvents = [];
      }
      if (holidaysData is List<Holiday>) {
        _selectedHolidays = holidaysData;
      } else {
        _selectedHolidays = [];
      }
      _selectedDate = day;
              ...
              children: <Widget>[
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    labelText: 'Jumlah $prefix Bulanan',
                  ),
        ...
        return EmployeePresenceCardWidget(
          ...
          buttonWidget: SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                foregroundColor: Colors.white, backgroundColor: Colors.red[600], shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(6),),
              ),
              onPressed: () {
  ...
  void _cancelAttendance(Presence presence) {
    Get.defaultDialog(
        ...
        confirm: ElevatedButton(
          style: ElevatedButton.styleFrom(
            foregroundColor: Colors.white, backgroundColor: Colors.blueAccent, shape:
  ...
  Future<void> _sendData(Presence presence) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        pd.hide();
        showAlertDialog("success", "Sukses", res['message'].toString(),
            dismissible: false,);
        Timer(
