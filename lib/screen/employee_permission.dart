...
class _EmployeePermissionScreenState extends State<EmployeePermissionScreen> {
  ...
  Future<void> _fetchPermissionData() async {
    try {
      ...
      final List<dynamic> permissions = result['data'] as List<dynamic>;

      final List<AbsentPermission> data = permissions
          .map(
              (json) => AbsentPermission.fromJson(json as Map<String, dynamic>),)
          .toList();
      setState(() {
        _permissions = data;
        _filteredPermission = data;
      });
    } catch (e) {
      showErrorDialog({
        'message': 'Kesalahan',
  ...
  SizedBox _cancelButton(String label, AbsentPermission permission) {
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          foregroundColor: Colors.white, backgroundColor: Colors.red, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
        ),
        onPressed: () {
  ...
  SizedBox _approveButton(AbsentPermission permission) {
    return SizedBox(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
          foregroundColor: Colors.white, backgroundColor: Colors.blueAccent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
        ),
        onPressed: () {
